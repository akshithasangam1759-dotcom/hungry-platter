...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: result.insertId, email, role: 'customer' }, process.env.JWT_SECRET || 'secret', { expiresIn: '7d' });
  ...
  try {
    ...
    if (!rows.length) {
      await db.query(
        ...
        [null, email, ip, device, 'failed']  // user_id is null since user doesn't exist
      );
    ...
    if (!match) {
      await db.query(
        'INSERT INTO login_logs (user_id, username, ip_address, device_info, status) VALUES (?,?,?,?,?)',
        [user.id, user.name, ip, device, 'failed']
      );
      return res.status(400).json({ message: 'Invalid credentials' });
    }
    ...
    await db.query(
      'INSERT INTO login_logs (user_id, username, ip_address, device_info, status) VALUES (?,?,?,?,?)',
      [user.id, user.name, ip, device, 'success']
    );
